...
@Component
class FriendFavoriteType @Autowired constructor(
    ...
    override val type: NotificationType = NotificationType.FRIEND_FAVORITE
    override fun createNotification(event: Event): Notification {
        ...
        val notification = Notification(event.type!!, "Your friend " + senderUser?.firstName +" "+ senderUser?.lastName + " added "+ hotelDto.name + " in favorite", user = senderUser, hotel = hotel, createdAt = dateProvider.getCurrentDateTime())
    ...
    private fun checkNotificationSettings(user: User): Boolean {
        ...
        return settingUser.notificationSetting?.pauseAll == false && settingUser.notificationSetting?.friendFavorite == false
    }
}